...
export const fetchAllCourses = createAsyncThunk(
    ...
    async ({ page = 1, search = '' }, { rejectWithValue }) => {
        try {
            const response = await api.get(`/admin/courses?page=${page}&search=${search}`);
            return response.data.data;
        } catch (error) {
    ...
    async (courseData, { rejectWithValue }) => {
        try {
            ...
            if (courseData.image && courseData.image.file) {
                ...
                if (courseData.oldPrice) formData.append('oldPrice', courseData.oldPrice);
...
export const updateCourse = createAsyncThunk(
    ...
    async ({ courseId, courseData }, { rejectWithValue }) => {
        try {
            ...
            const response = await api.patch(`/admin/courses/${courseId}`, dataToSend, config);
            return response.data.data.course;
        } catch (error) {
...
export const updateSubscription = createAsyncThunk(
    ...
    async ({ subscriptionId, subscriptionData }, { rejectWithValue }) => {
        try {
            const response = await api.patch(`/admin/subscriptions/${subscriptionId}`, subscriptionData);
            return response.data.data;
        } catch (error) {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(updateCourse.fulfilled, (state, action) => {
                if (Array.isArray(state.courses.list)) {
                    const index = state.courses.list.findIndex(course => course._id === action.payload._id);
                    if (index !== -1) {
            ...
            .addCase(createLesson.fulfilled, (state, action) => {
                const courseIndex = state.courses.list.findIndex(course => course._id === action.meta.arg.courseId);
                if (courseIndex !== -1) {
                    ...
                    course.totalDuration = (course.totalDuration || 0) + (action.payload.duration || 0);
                }
            })
            .addCase(updateLesson.fulfilled, (state, action) => {
                ...
                if (courseIndex !== -1) {
                    const course = state.courses.list[courseIndex];
                    if (Array.isArray(course.lessons)) {
                        const lessonIndex = course.lessons.findIndex(lesson => lesson._id === action.meta.arg.lessonId);
                        if (lessonIndex !== -1) {
            ...
            .addCase(deleteLesson.fulfilled, (state, action) => {
                const courseIndex = state.courses.list.findIndex(course => course._id === action.payload.courseId);
                if (courseIndex !== -1) {
                    const course = state.courses.list[courseIndex];
                    course.lessons = course.lessons.filter(lesson => lesson._id !== action.payload.lessonId);
                    course.totalLessons = (course.totalLessons || 0) - 1;
                    course.totalDuration = (course.totalDuration || 0) - (action.payload.lesson.duration || 0);
                }
            })
            // Subscriptions
